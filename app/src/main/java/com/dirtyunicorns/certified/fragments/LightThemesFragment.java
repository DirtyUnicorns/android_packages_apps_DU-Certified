...
public class LightThemesFragment extends AbstractThemeFragment {
    @Override
    public Call<java.util.List<Theme>> getThemeCall() {
        if (PreferenceManager.getDefaultSharedPreferences(getContext())
                .getBoolean(getString(R.string.freethemes_switch), false)) {
            return ItemDownloader.getThemesApi().getFreeLightThemes();
        } else {
            return ItemDownloader.getThemesApi().getLightThemes();
        }
    }
}
