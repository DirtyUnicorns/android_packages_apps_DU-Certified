package com.dirtyunicorns.certified.colorpicker;
...
public class ColorPickerView extends View {
	...
	public ColorPickerView(Context context, AttributeSet attrs) { this(context, attrs, 0); }
	...
	private int[] buildHueColorArray(){
		int[] hue = new int[361];
		int count = 0;
		for(int i = hue.length -1; i >= 0; i--, count++){ hue[count] = Color.HSVToColor(new float[]{i, 1f, 1f}); }
		return hue;
	}
	@Override
	...
	public boolean onTrackballEvent(MotionEvent event) {
		...
		if(event.getAction() == MotionEvent.ACTION_MOVE){
			...
			case PANEL_HUE:
				...
				break;
			case PANEL_ALPHA:
				if(!mShowAlphaPanel || mAlphaRect == null){ update = false;	}
				else{
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if(!mShowAlphaPanel){
			...
			else{ height = heightAllowed; }
		}
		setMeasuredDimension(width, height);
	}
	private int chooseWidth(int mode, int size){
		if (mode == MeasureSpec.AT_MOST || mode == MeasureSpec.EXACTLY) { return size;}
		else { return getPrefferedWidth(); }
	}
	private int chooseHeight(int mode, int size){
		if (mode == MeasureSpec.AT_MOST || mode == MeasureSpec.EXACTLY) { return size; }
		else { return getPrefferedHeight();	}
	}
	private int getPrefferedWidth(){
		int width = getPrefferedHeight();
		if(mShowAlphaPanel){ width -= (PANEL_SPACING + ALPHA_PANEL_HEIGHT);	}
	...
	public void setColor(int color, boolean callback){
		...
		mSat = hsv[1];
		mVal = hsv[2];
		if(callback && mListener != null){ mListener.onColorChanged(Color.HSVToColor(mAlpha, new float[]{mHue, mSat, mVal})); }
		invalidate();
	}
	public float getDrawingOffset(){ return mDrawingOffset; }
	...
	public boolean getAlphaSliderVisible() { return mShowAlphaPanel; }
	public void setSliderTrackerColor(int color){
		mSliderTrackerColor = color;
		mHueTrackerPaint.setColor(mSliderTrackerColor);
		invalidate();
	}
	public int getSliderTrackerColor(){	return mSliderTrackerColor; }
	...
	public String getAlphaSliderText(){	return mAlphaSliderText; }
}